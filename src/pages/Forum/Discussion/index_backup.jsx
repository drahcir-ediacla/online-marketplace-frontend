...
const Discussion = () => {
    ...
    useEffect(() => {
        if (repliedPostId && allPost.length > 0) { // Ensure allPost has data
            setTimeout(() => {
                expandRepliesToPost(repliedPostId);
            }, 3000);
        }
    }, [repliedPostId, allPost]); // Add allPost as a dependency
    ...
    const expandRepliesToPost = (repliedPostId) => {
        ...
        allPost.map(post => (
            // Loop through levelOne replies
            post?.replies?.forEach(levelOneReply => {
                // Check if this is the reply or contains the reply in deeper levels
                if (levelOneReply?.post_id === repliedPostId) {
                    // If the target post is levelOneReply, scroll to it
                    scrollToPostAfterExpand(levelOneReply.post_id);
                } else if (levelOneReply?.replies?.some(levelTwoReply => levelTwoReply?.post_id === repliedPostId || levelTwoReply?.replies?.some(levelThreeReply => levelThreeReply?.post_id === repliedPostId))) {
                    // Expand levelOne replies to show levelTwo
                    toggleShowMoreReplies(levelOneReply?.post_id);

                    // Loop through levelTwo replies
                    levelOneReply?.replies.forEach(levelTwoReply => {
                        if (levelTwoReply?.post_id === repliedPostId) {
                            // If the target post is levelTwoReply, scroll to it
                            scrollToPostAfterExpand(levelTwoReply.post_id);
                        } else if (levelTwoReply?.replies?.some(levelThreeReply => levelThreeReply?.post_id === repliedPostId)) {
                            // Expand levelTwo replies to show levelThree
                            toggleShowMoreReplies(levelTwoReply?.post_id);

                            // Scroll to levelThree reply after expansion
                            levelTwoReply.replies.forEach(levelThreeReply => {
                                if (levelThreeReply?.post_id === repliedPostId) {
                                    scrollToPostAfterExpand(levelThreeReply?.post_id);
                                }
                            });
                        }
                    });
